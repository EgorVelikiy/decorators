...
def decorator(old_function):
    ...
    def new_function(*args, **kwargs):
        with open('old_func_decorator', 'a', encoding='utf-8') as log_file:
            log_file.write(
                f'Function {old_function.__name__} started {datetime.now()} with args: {args}, {kwargs}\n')
            result = old_function(*args, **kwargs)
            log_file.write(f'And returned {result}\n')
